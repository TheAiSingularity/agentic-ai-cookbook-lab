...
- **Search:** N parallel SearXNG queries (free self-hosted meta-search
  over DuckDuckGo / Bing / Wikipedia / arXiv). Each sub-query's top-k
  snippets are LLM-summarized with inline citations.
- **Retrieve:** Hybrid BM25 + dense (OpenAI-compat `EMBED_MODEL`) + RRF
  (k=60); optional contextual chunking; optional cross-encoder rerank
  on top-50 → top-K.
...
- **Verify (CoVe):** Planner-tier model parses the answer into discrete
